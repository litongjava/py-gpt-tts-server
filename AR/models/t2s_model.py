...
class Text2SemanticDecoder(nn.Module):
  ...
  def infer_panel_batch_infer_with_flash_attn(
    ...
    **kwargs,
    ...
    x_list = [F.pad(item, (0, 0, 0, max_len - item.shape[0]), value=0) if item.shape[0] < max_len else item for item in
              x_list]
    x = torch.stack(x_list, dim=0)

    bert_features_list = [self.bert_proj(item.transpose(0, 1)) for item in bert_feature]
    bert_features_list = [F.pad(item, (0, 0, 0, max_len - item.shape[0]), value=0) if item.shape[0] < max_len else item
  ...
  def infer_panel_with_flash_attn_only(
    ...
    **kwargs
    ...
    xy_attn_mask = torch.concat([x_attn_mask_pad, y_attn_mask], dim=0).unsqueeze(0).expand(bsz * self.num_head, -1,
                                                                                           -1).view(bsz, self.num_head,
                                                                                                    src_len,
                                                                                                    src_len).to(
      x.device)
    new_attn_mask = torch.zeros_like(xy_attn_mask, dtype=x.dtype)
